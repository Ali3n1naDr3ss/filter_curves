...
LGB_spectrum = '/Users/user/Documents/filter_curves/data_files/REBELS-05_1dspec_wRMS.dat'
LGB_wavelength = []
LGB_flux = [] #nJy
with open(LGB_spectrum,'r', encoding='utf-8-sig') as file:
    lines = file.readlines()[2:]
    for line in lines:
        columns = line.split()
        LGB_wavelength.append(1000*float(columns[0])) # convert to Angstrom
        LGB_flux.append(float(columns[1]))
        


ax4.plot(LGB_wavelength, LGB_flux, label='LGB Spectrum', color=nipy_spectral(1))
...
plt.plot(LGB_wavelength, LGB_flux, label='Spectrum', color=nipy_spectral(1))
plt.plot(x,y, label='Lyman Limit', color=nipy_spectral(0.5))
